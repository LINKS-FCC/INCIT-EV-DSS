...
def P_module(abs_path,Number_of_CS, VP_matrix_short, VP_matrix_medium, VP_matrix_long,Zone_type, Expected_EV, TRNP, ML, PP, PPP,SPP, SPPP, TDP,AY, CIY):
    ############Original
    ...
    #Added by Klemen Knez in 23.06.2021 TODO: Hardcoded first row
    Stays = pd.read_csv(abs_path + 'impact_kw_mean_7_days.csv', header=None, index_col=None).iloc[0,:]*100*float(Expected_EV)
    ...
    P_profile_database = pd.read_excel(abs_path+'/P_module_generated_profiles.xlsx')
    RES_profile_database = pd.read_excel(abs_path+'/Solar_PP_generation_profile.xlsx',header=None, dtype = float)
    if not SPP:
        SPPP = 0
    if not (((P_profile_database.iloc[:,0] == Zone_type)*1)+((P_profile_database.iloc[:,1] == int(TRNP))*1)+((P_profile_database.iloc[:,2] == TDP)*1)).max() == 3:
        ...
        P_profile_tmp = pd.read_excel(abs_path + '/' + PPP, header=None)
        ...
        P_profile = (P_profile_tmp * float(ML) / P_profile_database.iloc[(((P_profile_database.iloc[:,0] == Zone_type) * 1) + ((P_profile_database.iloc[:,1] == int(TRNP)) * 1) + ((P_profile_database.iloc[:,2] == TDP)*1)).idxmax(), 3]).reset_index(drop=True) \
                    - RES_profile_database.iloc[0,:] * int(SPPP)
    ...
    [Voltage_drop_KPI, Power_demand_KPI, GR_KPI, Voltage_events_KPI, Power_events_KPI, Losses_KPI, ZSCR_KPI, Peak_deviation_KPI] = PM.Power_KPI(P_profile,Stays,Voltage_coef, Nominal_voltage, TRNP, RES_profile_database.iloc[0,:] * int(SPPP))

    return Occupancy, Borders, Stays, RHC, Voltage_drop_KPI, Power_demand_KPI, GR_KPI, Voltage_events_KPI, Power_events_KPI