...
def Inputs():
    ...
    while True:
        ...
        if event == "Submit":
            ...
            #Execute CI module
            if int(values['CIY']) == 1:
                [New_CSs, Number_of_CS, VP_matrix_short, VP_matrix_medium, VP_matrix_long, ROI, CPPV] = CI.CI_module(values[0], values[1], values[2],values['ECSF'],values['ECSFAC'],values['ECSS'],values['CIY'])
            else:
                [Number_of_CS, VP_matrix_short,VP_matrix_medium, VP_matrix_long, CPPV] = CI.CI_module(values[0], values[1], values[2], values['ECSF'],values['ECSFAC'], values['ECSS'],values['CIY'])
            #Execute P module
            ...
            #Present results
            if int(values['CIY']) == 1:
                Outputs(values[0],New_CSs, Occupancy, Borders, Stays, RHC, values['CIY'], values['ECSF'],values['ECSFAC'], values['ECSS'], values[3], CPPV, Voltage_drop_KPI, Power_demand_KPI, GR_KPI, Voltage_events_KPI, Power_events_KPI)
            else:
                Outputs(values[0],([0,0,0]), Occupancy, Borders, Stays, RHC, values['CIY'], values['ECSF'],values['ECSFAC'], values['ECSS'], values[3], CPPV, Voltage_drop_KPI, Power_demand_KPI, GR_KPI, Voltage_events_KPI, Power_events_KPI)
            break
...
def Outputs(abs_path,New_CSs, Occupancy, Borders, Stays, RHC, CIY, ECSF, ECSFAC, ECSS, TNP, CPPV, Voltage_drop_KPI, Power_demand_KPI, GR_KPI, Voltage_events_KPI, Power_events_KPI):
